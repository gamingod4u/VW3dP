...
public class WallController : MonoBehaviour 
{
	...
	void Update ()
	{
		if (rotateSpeed != 0.0f) 
		{
			...
			// lowest
			//Debug.Log("Name: " + contentGrid[lowestIdx].obj[0].transform.name + " Rotation: " + contentGrid[lowestIdx].rotation);
			if (contentGrid[lowestIdx].rotation > 150 && contentGrid[lowestIdx].rotation < 170 && rotateSpeed < 0)
			{
				// Unload this column
			...
			//highest
			if (contentGrid[highestIdx].rotation > 150 && contentGrid[highestIdx].rotation < 180 && rotateSpeed > 0)
			{
				// Unload this column
